...
let gridIndices n =
    [for i in 0 .. n - 1 -> [for j in 0 .. n - 1 -> (i, j)]]

let isInBounds size point =
    fst point >= 0 && snd point >= 0 && fst point < size && snd point < size

let contains lst elem =
    (lst |> List.filter (fun x -> x = elem) |> List.length) <> 0
...
let updateGrid (grid: int list list) index =
    ...
    if grid.[fst index].[snd index] >= 10 then
        ...
        |> List.mapi (fun i lst -> lst
                                   |> List.mapi (fun j x -> if (i, j) = index then -1
                                                            else if contains adj (i, j) && grid.[i].[j] <> -1 then
                                                                grid.[i].[j] + 1
                                                            else grid.[i].[j]))
    else grid
...
let rec flash grid =
    ...
    let numFlashes = updatedGrid |> List.fold (@) [] |> List.filter ((<=) 10) |> List.length

    if numFlashes = 0 then
        updatedGrid |> List.map (List.map (fun i -> if i = -1 then 0 else i))
    else
        flash updatedGrid

// Iterates the grid n times and then returns the total number of flashes.
let rec numFlashes n grid =
    let numZeros = List.fold (@) [] >> List.filter ((=) 0) >> List.length
...
let rec findSyncHelper n grid =
    let allZeros = List.fold (@) [] >> List.forall (fun i -> i = 0)
